...
const val CLASS_SCORE_END_INDEX = 8
...
object Detector {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    private fun CoroutineScope.modelOutputLooper(modelOutput: ModelOutputReshaped) = produce {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    private fun CoroutineScope.classScoreExtractor(
        isFrontJaw: Boolean,
        ...
        val classScores = if (isFrontJaw) {
            // For the front jaw, we consider 8 classes (scores at indices 4 through 12).
            modelOutput.slice(CLASS_SCORE_START_INDEX until CLASS_SCORE_END_INDEX_FRONT_JAW)
        } else {
            // For a side jaw, we only consider the first 4 classes (scores at indices 4 through 8).
            modelOutput.slice(CLASS_SCORE_START_INDEX until CLASS_SCORE_END_INDEX)
        }
        send(classScores)
    }

    @OptIn(ExperimentalCoroutinesApi::class)
    private fun CoroutineScope.normalizeDetectedBox(modelOutput: ReceiveChannel<FloatArray>) =
        produce<Box> {
            val modelOutput = modelOutput.receive()
            val normalizeConstant: Float = INPUT_IMAGE_SIZE.toFloat()
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    private fun CoroutineScope.validToothBoxExtractor(
        maxScore: Float,
        maxScoreIndex: Int,
        modelOutput: ReceiveChannel<FloatArray>
    ) = produce {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    private fun CoroutineScope.processOutput(
        isFrontJaw: Boolean,
